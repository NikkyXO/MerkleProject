...
contract MerkleAirdrop is  ReentrancyGuard {  
    ...
    constructor(bytes32 _root, address token, address _bayc) {
     root = _root;
    ...
    function claim(bytes32[] calldata _proof, uint amount) external nonReentrant {
        address claimer = msg.sender;
         require(bayc.balanceOf(claimer) > 0, "Must own a BAYC NFT");
        ...
        require(tokenAddress.balanceOf(address(this)) > amount, 'AIRDROP CLAIM: No token to release by airdropper');
        tokenAddress.safeTransfer(claimer, amount);
    }
